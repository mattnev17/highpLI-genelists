...
load("../GeneDiscovery/output/Data/universe_df.rda")
...
ddd19features <- read_tsv("gene_features.tab") 

ddd19 <- read_tsv("extended_denovoWEST_results.tab") %>% 
...
#Get DD2GP genes
ddg2p <- read_csv("DDG2P_12_11_2019.csv") %>% 
...
#Get OMIM genes
omimGenes <- read_tsv("/Volumes/ddd0/mn7/morbidmap_2019_10_08.txt", skip = 4, comment = '#', col_types = 'ccic',
...
#saveRDS(omimGenes, "omimGenes.rds")
#omimGenes = readRDS("omimGenes.rds")
...
omim <- read_tsv("/Volumes/ddd0/mn7/genemap2_2019_10_08.txt", skip = 3) %>% 
...
clinvarVCFfiltered <- read_tsv("clinvar_20191003.vcf.gz", comment = '#', col_names = F, col_types = 'cccccccccccccc') %>% 
...
llik3_genes <- read_tsv("forMatt_DDgene_likratio_gt3_2019_11_12.txt") %>% 
  select(hgnc_id = num_hgnc_id)

#Core essential genes from moffat G3 paper
ceg2 <- read_tsv("CEG2.txt", col_names = c("gene", "hgnc_id"), col_types = 'cc') %>% 
  mutate(hgnc_id = str_remove(hgnc_id, "HGNC:"))

cellEssential3hit <- read_tsv("TableS3.txt") %>% 
...
hap1Bayes <- read_tsv("TableS3.txt") %>% 
  select(GENE, BF_HAP1) %>% 
  drop_na() %>% 
  convertGeneToHGNC(geneCol = "GENE") %>% 
  ...
  distinct(hgnc_id, .keep_all = T) %>% 
  select(-GENE)
kbm7Bayes <- read_tsv("TableS3.txt") %>% 
  select(GENE, BF_KBM7) %>% 
  drop_na() %>% 
  convertGeneToHGNC(geneCol = "GENE") %>% 
  #Select highest bayes factor on gene duplicates
...
cancerCellLines <- read_csv("Supplementary Table 2 - Fitness genes and pathway enrichments.csv") %>%
...
gtexMedianother <- read_tsv("GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct.gz", skip = 2) %>% 
...
cosmicCancerGenes <- read_tsv("cosmicCancerGenes.tsv") %>% 
...
reproductionGenes <- read_tsv("sexualReproductionGO.txt", col_names = c("gene", "GO", "organism")) %>% 
  distinct(gene) %>% 
  convertGeneToHGNC(geneCol = "gene") %>% 
  select(hgnc_id) %>% 
  unique()
...
kimLists <- read_tsv("geneListsKim19.txt", comment = '#') 
...
inWebCentrality <- read_tsv("inWebCentrality.txt", comment = '#') %>% 
  left_join(hgncIDs, by =c("ENTREZ" = "entrez_gene_id") ) %>% 
  rename(inweb_closeness = Closeness) %>% 
  select(hgnc_id, inweb_closeness) %>% 
  unique()
greeneCentrality <- read_tsv("greeneCentrality.txt", comment = '#') %>% 
  left_join(hgncIDs, by =c("ENTREZ" = "entrez_gene_id") ) %>% 
...
drugTargets <- read_tsv("gene_lists/lists/fda_approved_drug_targets.tsv", col_names = "gene") %>% 
  convertGeneToHGNC(geneCol = "gene") 
#mouseEssential <- read_tsv("gene_lists/lists/mgi_essential.tsv", col_names = "gene") %>% 
#  convertGeneToHGNC(geneCol = "gene")
clinGenHaploinsufficient <- read_tsv("gene_lists/lists/clingen_level3_genes_2018_09_13.tsv", col_names = "gene") %>% 
  convertGeneToHGNC(geneCol = "gene")
olfactoryReceptor <- read_tsv("gene_lists/lists/olfactory_receptors.tsv", col_names = "gene") %>% 
  convertGeneToHGNC(geneCol = "gene")
exacHomozygous_lof_tolerant <- read_tsv("gene_lists/lists/homozygous_lof_tolerant_twohit.tsv", col_names = "gene") %>% 
  convertGeneToHGNC(geneCol = "gene")
nearestGWAS_peakGene <- read_tsv("gene_lists/lists/gwascatalog.tsv", col_names = "gene") %>% 
  convertGeneToHGNC(geneCol = "gene")

#Num probes in UKBB for that analysis
#UKBB probe counts
probes <- read_csv("../ukbb/Axiom_UKB_WCSG.na34.annot.csv", comment = "#") %>% 
  select(affy_snp_id = `Affy SNP ID`, genes = `Associated Gene`) %>% 
  filter(genes!= "---") %>% 
  separate(genes, into = c("gene"), sep = " /// ", extra = "drop") %>% 
  separate(gene, into = c("transcript", "csq", "c", "d", "gene", "f", "g"), sep = " // ") %>% 
  drop_na() %>% 
  filter(gene != "---" & csq != "downstream" & csq != "upstream") %>% 
  group_by(gene) %>% 
  summarize(n_probes = n()) %>% 
  convertGeneToHGNC(geneCol = "gene") %>% 
  select(-gene) %>% 
  distinct(hgnc_id, .keep_all = T)
...
gtexMedian <- read_tsv("GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct.gz", skip = 2) %>% 
...
brainspanSamples <- read_csv("brainspan/columns_metadata.csv") %>% 
...
brainspanRows <- read_csv("brainspan/rows_metadata.csv", col_types = 'iicci') %>% 
  drop_na() %>% 
...
brainspanMatrix <- read_csv("brainspan/expression_matrix.csv", col_names = c("row_num", brainspanSamples$sampleID), col_types = cols(.default = col_double())) %>% 
